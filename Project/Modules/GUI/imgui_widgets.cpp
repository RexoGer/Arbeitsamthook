...
{
    return window->GetIDNoKeepAlive(axis == ImGuiAxis_X ? xor("#SCROLLX") : xor("#SCROLLY"));
}
...
bool ImGui::Checkbox(const char* label, bool* v, ImVec4 DetailColor)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect check_bb(
        window->DC.CursorPos + ImVec2(0, 3),
        window->DC.CursorPos + ImVec2(
            label_size.y + style.FramePadding.x * 6,
            label_size.y + style.FramePadding.y / 2));

    ItemSize(check_bb, style.FramePadding.y + 6);
    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(
        window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 0),
        window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 0) + label_size);
    if (label_size.x > 0)
    {
        ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight()), style.FramePadding.y);
        total_bb = ImRect(ImMin(check_bb.Min, text_bb.Min), ImMax(check_bb.Max + ImVec2(0, 20), text_bb.Max));
    }
    ...
    ImU32 DarkColor = GetColorU32(ImVec4(0.25f, 0.25f, 0.25f, 1.00f));
    ImU32 ActiveColor = ColorConvertFloat4ToU32(DetailColor);
    
    if (*v) {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 8, check_bb.Min.y + 9), CircleRadius, ActiveColor, NumSegments);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 5, check_bb.Min.y + 9 - CircleRadius), ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 8, check_bb.Min.y + 9 + CircleRadius), ActiveColor);
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 5, check_bb.Min.y + 9), CircleRadius, ActiveColor, NumSegments);

        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 8, check_bb.Min.y + 9), CircleRadius - 2, GetColorU32(ImVec4(0.1f, 0.1f, 0.1f, 1.0f)), NumSegments);
    }
    else {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 8, check_bb.Min.y + 9), CircleRadius, DarkColor, NumSegments);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 5, check_bb.Min.y + 9 - CircleRadius), ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 8, check_bb.Min.y + 9 + CircleRadius), DarkColor);
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 5, check_bb.Min.y + 9), CircleRadius, DarkColor, NumSegments);

        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 5, check_bb.Min.y + 9), CircleRadius - 2, GetColorU32(ImVec4(0.1f, 0.1f, 0.1f, 1.0f)), NumSegments);
    }
    if (label_size.x > 0.0f)
...
bool ImGui::Joystick(ImVec2 Position, const char* Id)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 15);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    ImVec2 Pos = ImVec2((check_bb.Max.x - check_bb.Min.x) * Position.x + check_bb.Min.x, (check_bb.Max.y - check_bb.Min.y) * Position.y + check_bb.Min.y);
    Pos.y += check_bb.Max.y - check_bb.Min.y;
    window->DrawList->AddCircle(Pos, 4, check_col);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return true;
}
bool ImGui::Trigger(float Position, const char* Id)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 0);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    float Height = check_bb.Max.y - check_bb.Min.y;
    Height *= (Position * -1 + 1);
    ImVec2 Top = ImVec2(check_bb.Min.x, check_bb.Min.y + Height);
    ImVec2 Bottom = ImVec2(check_bb.Max.x, check_bb.Max.y);
    window->DrawList->AddRectFilled(Top, Bottom, check_col);
...
bool ImGui::Hotkey(const char* label, int* k, ImVec4 col, const ImVec2& size_arg)
{
    ...
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(col), true, style.FrameRounding);

    if (*k != 0 && g.ActiveId != id) {
        strcpy_s(buf_display, KeyCodes[*k]);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        grab_bb.Max.x = grab_bb.Max.y - grab_bb.Min.y + grab_bb.Min.x;
    ...
    ImVec4 oTextCol = colors[ImGuiCol_Text];
    ImVec4 nTextCol = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    float Alpha = ((*(float*)p_data - *(float*)p_min) / (*(float*)p_max - *(float*)p_min) * -1 + 1) * 0.5 + 0.5;
    colors[ImGuiCol_Text] = ImVec4(0.0f, 0.0f, 0.0f, 0.75f);

    // Text Drop Shadow
    frame_bb.Max.x += 1; frame_bb.Max.y += 1; frame_bb.Min.x += 1; frame_bb.Min.y += 1;
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.975f, 0.5f));
    frame_bb.Max.x -= 1; frame_bb.Max.y -= 1; frame_bb.Min.x -= 1; frame_bb.Min.y -= 1;

    colors[ImGuiCol_Text] = nTextCol;
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.975f, 0.5f));
    colors[ImGuiCol_Text] = oTextCol;
